...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Car Parking Multiplayer Mod Menu</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:red;\">Modded by chr1s | Join the discord</p></p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by chr1s#4191, To ensure you're playing on the official mod, and not a tampered one, make sure you got this mod off offical modding forums!"),ToastLength::LENGTH_LONG);
    Toast(env,ctx,OBFUSCATE("Modded by chr1s#4191, To ensure you're playing on the official mod, and not a tampered one, make sure you got this mod off offical modding forums!"),ToastLength::LENGTH_LONG);
    initValid = true;
}