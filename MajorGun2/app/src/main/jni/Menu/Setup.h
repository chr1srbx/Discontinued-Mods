...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Major Gun 2 Mod Menu</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:red;\">Modded by chr1s | I forgot to replace the mod menu name on Team Factory 2 fuck</p></p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by chr1s#4191"),ToastLength::LENGTH_LONG);
